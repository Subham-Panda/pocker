...
void pivot_root_helper() {
    const char MOUNT_DIR[] = "./pocker_alpine";
    const char OLD_ROOT[] = "./oldroot";

    assertCondtion(mount(MOUNT_DIR, MOUNT_DIR, NULL, MS_BIND | MS_PRIVATE | MS_REC, NULL) != -1, "Failed to mount MS_PRIVATE on MOUNT_DIR");
    assertCondtion(chdir(MOUNT_DIR) != -1, "Failed to change directory to MOUNT_DIR");
    assertCondtion(mkdir(OLD_ROOT, 0755) != -1 || errno == EEXIST, "Failed to make OLD_ROOT directory");
    assertCondtion(pivot_root(".", OLD_ROOT) != -1, "Pivot root has failed");
    assertCondtion(chdir("/") != -1, "Failed to change diretcory to the new /");
    assertCondtion(umount2(OLD_ROOT, MNT_DETACH) != -1, "Failed to unmount OLD_ROOT");
    assertCondtion(rmdir(OLD_ROOT) != -1, "Failed to remove old root");
}
